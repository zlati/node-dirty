...
var Dirty = exports.Dirty = function (path) {
    ...
    var d = new Date();
    this.pathDate = d.getFullYear() + '' + (d.getMonth() + 1);
...
//add another function to seed the DB
    if (tableCheck) {
        ...
        pg.connect(process.env.DATABASE_URL, function (err, client) {
            var queryCreateTable = client.query('CREATE TABLE IF NOT EXISTS ' + self.path + ' (tname text NOT NULL, jkey text NOT NULL, jval text)', function (err, result) {
                if (err) {
...
Dirty.prototype._insertSet = function (key, val) {
    ...
    pg.connect(process.env.DATABASE_URL, function (err, client) {
        var deletedC = 0;
        var qdelete = client.query('DELETE FROM ' + self.path + ' WHERE jkey=$1 AND tname=$2', [key, self.pathDate], function (err, result) {
            if (err) {
                console.error('Error running query 3', err);
            }
            deletedC = result.rowCount;

        });
        qdelete.on('end', function () {
            if (deletedC > 0) {
//                console.log('Deleted: no match found for ' + self.path + ' tname:' + self.pathDate + ' key:' + key + ' val: '+ val);
            }
            else {
//                console.log('Deleted: match found for ' + self.path + ' tname:' + self.pathDate + ' key:' + key + ' val: '+ val);
            }
        });

        if (val == null || val === false) {
            console.log('NULL value "' + val + '" for key "' + key + '" not inserted into DB "' + self.path + '"');
        } else {
            if (typeof val === 'object') {
                val = JSON.stringify(val);
            }
            //console.log('Db: ' + self.path + '; type ' + typeof val + '; key: ' + key + '; val: ', val);          
            var query = client.query('INSERT INTO ' + self.path + ' (tname, jkey, jval) VALUES ($1, $2, $3)', [self.pathDate, key, val], function (err, result) {
                if (err) {
                    console.error('Error running query 4', err);
                }
            });
            query.on('end', function () {
//            if(self.path.search('alerts')>0){
//                 console.log('Inserted into ' + self.path + ' tname:' + self.pathDate + ' key:' + key + ' val: ', val);
//            }
                //  console.log('Inserted into ' + self.path + ' tname:' + self.pathDate + ' key:' + key + ' val: ', val);
                client.end();
            });
