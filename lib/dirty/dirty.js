...
var self=this;
    ...
    pg.connect(process.env.DATABASE_URL, function (err, client) {
        var queryCheckTable = client.query('SELECT EXISTS(SELECT * FROM information_schema.tables WHERE table_schema=public AND table_name=$1 );', [self.path], function (err, result) {
            if (err) {
    ...
    } else
    {
        ...
        pg.connect(process.env.DATABASE_URL, function (err, client) {
            var queryCreateTable = client.query('CREATE TABLE ' + self.path + ' (jkey text NOT NULL, jval text, CONSTRAINT key PRIMARY KEY (jkey))', function (err, result) {
                if (err) {
  ...
  var self = this, buffer = '';
    ...
    pg.connect(process.env.DATABASE_URL, function (err, client) {
        ...
        queryLoadTable.on('row', function (row) {
            self._updateDocs(row.jkey, row.jval);
        });
