...
Dirty.prototype.set = function(key, val, cb) {
    var self=this;
            ...
            pg.connect(process.env.DATABASE_URL, function (err, client) {
                ...
                var query = client.query('INSERT INTO ' + self.path + ' (jkey, jval) VALUES ($1, $2)', [key, JSON.stringify(that._docs[key])], function (err, result) {
                    if (err) {
       ...
       if (self.path) {
        pg.connect(process.env.DATABASE_URL, function (err, client) {
            ...
            queryLoadTable.on('row', function (row) {
                self.set(row.jkey, JSON.parse(row.jval));
            });
