...
Dirty.prototype._load = function() {
  ...
  pg.connect(process.env.DATABASE_URL, function (err, client) {
            var query = client.query('CREATE TABLE IF NOT EXISTS ' + self.path + ' (key text NOT NULL UNIQUE, val text)', function (err, result) {
                if (err) {
...
////
            var query = client.query("SELECT key, val FROM " + self.table + "");
