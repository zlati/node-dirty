...
Dirty.prototype._load = function() {
  ...
  pg.connect(process.env.DATABASE_URL, function (err, client) {
        ...
        var queryCheckTable = client.query('SELECT EXISTS(SELECT * FROM information_schema.tables WHERE table_schema=$1 AND table_name=$2);',["public",self.path], function (err, result) {
            if (err) {
                console.error('1 error running query  ');
            }

            console.log('Checking for table ' + self.path);
        });

        queryCheckTable.on('row', function (row) {_flush
            console.log("Table " + self.path + " exists: ",row.exists.toString());
            if (row.exists.toString() != "false") {
                console.log('Table ' + self.path + ' is already existing');
            }
            else {
                 console.log('Try to create table ' + self.path + '');
                var queryCreateTable = client.query('CREATE TABLE ' + self.path + ' (key text NOT NULL, val text, CONSTRAINT key PRIMARY KEY (key))', function (err, result) {
                    if (err) {
                         console.error('1.1 error running query  ');
                    }
        ...
        if (self.path !== undefined){
            ...
            pg.connect(process.env.DATABASE_URL, function (err, client) {
                ...
                var query = client.query('INSERT INTO ' + self.path + ' (key, val) VALUES ($1, $2)', [key, that._docs[key]], function (err, result) {
                    if (err) {
                        console.error('2 error running query');
                    }
                });
