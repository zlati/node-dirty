...
Dirty.prototype._load = function() {
  ...
  pg.connect(process.env.DATABASE_URL, function (err, client) {
            var query = client.query('CREATE TABLE IF NOT EXISTS ' + self.path + ' (key text NOT NULL, val text, CONSTRAINT key PRIMARY KEY (name))', function (err, result) {
                if (err) {
                    return console.error('1 error running query', err);
                }

                console.log('Using  table ' + self.path);
            });

////
            var query = client.query("SELECT key, val FROM " + self.path + "");
