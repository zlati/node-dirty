...
Dirty.prototype._load = function() {
  ...
  pg.connect(process.env.DATABASE_URL, function (err, client) {
        ...
        var queryCheckTable = client.query('SELECT EXISTS(SELECT * FROM information_schema.tables WHERE table_schema = "public" AND table_name = ' + self.path + ');', function (err, result) {
            if (err) {
                return console.error('1 error running query  ', err);
            }
        ...
        queryCheckTable.on('row', function (row) {
            ...
            if (row.exists == 'f') {
                var queryCreateTable = client.query('CREATE TABLE IF NOT EXISTS ' + self.path + ' (key text NOT NULL, val text, CONSTRAINT key PRIMARY KEY (key))', function (err, result) {
                    if (err) {
                        return console.error('1 error running query  ', err);
                    }
                ...
                console.log('Table ' + self.path + ' is already existing');
            }
        });

////
            var queryLoadTable = client.query("SELECT key, val FROM " + self.path + "");
  ...
  for (var i = 0; i < length; i++) {
    ...
    if (Array.isArray(key)) {
      ...
      key = key[0];
        ...
        var that=this;
    console.log("t4 ??? will insert "+self.path);
        ...
        if (self.path !== undefined){
            ...
            pg.connect(process.env.DATABASE_URL, function (err, client) {
                ...
                var query = client.query('INSERT INTO ' + self.path + ' (key, val) VALUES ($1, $2)', [key, that._docs[key]], function (err, result) {
                    if (err) {
                        console.error('2 error running query', err);
                    }
                });
