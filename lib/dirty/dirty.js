...
Dirty.prototype._prepareDB = function () {
    ...
    } else
    {
        ...
        pg.connect(process.env.DATABASE_URL, function (err, client) {
            var queryCreateTable = client.query('CREATE TABLE ' + self.path + ' (jkey text NOT NULL, jval text, CONSTRAINT jkey PRIMARY KEY (jkey))', function (err, result) {
                if (err) {
      ...
      key = key[0];
        ...
        if (self.path){
            pg.connect(process.env.DATABASE_URL, function (err, client) {
                ...
                var query = client.query('INSERT INTO ' + self.path + ' (jkey, jval) VALUES ($1, $2)', [key, that._docs[key]], function (err, result) {
                    if (err) {
