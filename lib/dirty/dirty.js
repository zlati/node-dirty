...
Dirty.prototype._insertSet = function (key, val) {
    ...
    pg.connect(process.env.DATABASE_URL, function (err, client) {
        ...
        client.query('DELETE FROM ' + self.path + ' WHERE jkey=$1 and tname=$2', [key,self.path], function (err, result) {
            if (err) {
                console.error('Error running query 3', err);
            }
        });
        var query = client.query('INSERT INTO ' + self.path + ' (tname, jkey, jval) VALUES ($1, $2, $3)', [self.path, key, JSON.stringify(val)], function (err, result) {
            if (err) {
